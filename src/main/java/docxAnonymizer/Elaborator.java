 ...
 *  - debug:              variabile statica booleana, a default false, se settata a true attiva utili stampe di routine
 */
public class Elaborator {
	transient private List<R> runNodes;
	...
	public Elaborator(List<Object> runNodes, List<Persona> persone, List<Persona> keepUnchanged) {
		...
		readConfig("config" + File.separator + "keep_unchanged.txt", true);
	...
	public Elaborator(List<Object> runNodes, List<Persona> persone, List<Persona> keepUnchanged,
		...
		readConfig(keepUnchangedExprFile, false);
	}
	...
	public PlainTexts getPlainTexts() {
		return plainTexts;
	}

	public void setPlainTexts(PlainTexts plainTexts) {
	...
	public List<Persona> getKeepUnchanged() {
		return keepUnchanged;
	}

	public String getToKeepViaConfig() {
		return toKeepViaConfig;
	}
	 ...
	 */
	private void readConfig(String f_name, Boolean fromRes) {
		StringBuilder sb = null;
		String toKeep;
		try {
			BufferedReader bf = null;
			if (fromRes)
				bf = new BufferedReader(new InputStreamReader(Objects.requireNonNull(getClass().getClassLoader().getResourceAsStream(f_name))));
			else
			...
			while(toKeep != null) {
				...
				if(toKeep != null)
					sb.append("|")
							.append(Pattern.quote(toKeep.trim()))
							.append("|")
							.append(Pattern.quote(toKeep.trim().toUpperCase()));
			}
			bf.close();
		} catch (IOException e) {
			e.printStackTrace();
	...
	/**
	 * Nota: il metodo work e' bene che sia invocato tramite la classe Worker, non direttamente dal main.
	 *
	...
	private void enableDictionaries() {
		...
		try {
			...
			BufferedReader bf = new BufferedReader(new InputStreamReader(Objects.requireNonNull(getClass().getClassLoader().getResourceAsStream(f_name))));
	...
	public void updateUnchangeableViaConfig(List<EntryPoint> unchangeable, int index) {
	...
	public static void setDebug(boolean debug) {
		Elaborator.debug = debug;
	}

	/**
	 * "preprocess" e' inizializza le strutture dati senza effettuare la minimizzazione; il suo workflow:
	 *  - inizializza i dizionari se necessario
	 *  - effettua il processamento dei nodi docx per ottimizzare la minimizzazione, utilizzando una regex generale per
	 *    i nominativi
	 *
	 * @return lista di StringBuilder contenenti blocchi di testo semanticamente atomici
	 */
	public List<StringBuilder> preprocess() {
		...
		return s_preMinimization;

	}

	/**
	 * "postprocess" e' il metodo principale della classe Elaborator; il suo workflow:
	 *  - aggiorna il contenuto dei nodi docx
	 */
	public void postprocess(List<StringBuilder> s_postMinimization, List<EntryPoint> eps) {

		// sostituisco la variabile plainTexts corrente con la sua versione equivalente post-minimizzazione
		plainTexts.setPlainTexts(s_postMinimization);

		// inserisco entry points calcolati in remoto
		plainTexts.updateDocxNodes(plainTexts.getEntryPoints(), eps);
